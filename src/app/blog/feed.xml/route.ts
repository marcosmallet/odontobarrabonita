...
export function GET() {
  ...
  const items = posts.map((post) => `<item><title>${escapeXml(post.title)}</title><description>${escapeXml(post.description)}</description><link>${blogPostUrl(post.slug)}</link><guid isPermaLink="true">${blogPostUrl(post.slug)}</guid><pubDate>${new Date(`${post.publishedAt}T12:00:00Z`).toUTCString()}</pubDate></item>`).join("");
